...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b bg-white">
      <nav className="relative mx-auto max-w-7xl px-4 py-4 flex items-center justify-between">
        {/* Lines */}
        <div className="absolute inset-y-0 left-1/2 -z-10 w-screen -translate-x-1/2">
          <div className="absolute inset-x-0 top-0 border-t border-black/10"></div>
          <div className="absolute inset-x-0 top-2 border-t border-black/10"></div>
          <div className="absolute inset-x-0 bottom-0 hidden border-b border-black/10 group-last/row:block"></div>
          <div className="absolute inset-x-0 bottom-2 hidden border-b border-black/10 group-last/row:block"></div>
        </div>
        ...
        >
          {/* SVG icon */}
          <svg width="50" height="34" className="h-9 overflow-visible">
            <g
              opacity="1"
              style={{ transform: "none", transformOrigin: "17px 17px" }}
            >
